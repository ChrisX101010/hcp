...
impl MeshNode {
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        let adv = node.clone();
        tokio::spawn(async move {
            if let Err(e) = adv.advertise().await {
                error!("mDNS advertise: {}", e);
            }
        });

        let disc = node.clone();
        tokio::spawn(async move {
            if let Err(e) = disc.discover().await {
                error!("mDNS discover: {}", e);
            }
        });
        ...
        tokio::spawn(async move {
            loop {
                if let Ok((stream, addr)) = listener.accept().await {
                    let n = acc.clone();
                    tokio::spawn(async move {
                        if let Err(e) = n.handle_incoming(stream, addr).await {
    ...
    async fn advertise(&self) -> anyhow::Result<()> {
        ...
        let txt: HashMap<String, String> =
            [("advert".to_string(), serde_json::to_string(&advert)?)].into();
        let ip: IpAddr = Ipv4Addr::LOCALHOST.into();

        let service =
            ServiceInfo::new("_hcp._tcp.local.", &self.id, safe_host, ip, self.port, txt)?
                .enable_addr_auto();
    ...
    async fn discover(&self) -> anyhow::Result<()> {
        ...
        loop {
            match browser.recv_timeout(Duration::from_secs(1)) {
                        ...
                        if let Some(prop) = info.get_properties().get("advert") {
                            if let Some(val_bytes) = prop.val() {
                                if let Ok(val_str) = std::str::from_utf8(val_bytes) {
                                    if let Ok(mut adv) =
                                        serde_json::from_str::<NodeAdvertisement>(val_str)
                                    {
                ...
                Ok(_) => {}
                Err(flu) => {
                    if matches!(flu, flume::RecvTimeoutError::Disconnected) {
                        break;
                    }
                }
            }
    ...
    async fn connect_to_peer(&self, adv: &NodeAdvertisement) -> anyhow::Result<()> {
        ...
        let hello = Message::new(
            &self.id,
            serde_json::json!({"type":"hello","node_id":self.id}),
        );
        if let Err(e) = conn.send(&hello).await {
            warn!("Failed hello to {}: {}", adv.node_id, e);
            return Ok(());
        }
        {
                ...
                match tokio::time::timeout(Duration::from_secs(30), conn.recv()).await {
                    Ok(Ok(msg)) => {
                        if msg.payload.get("type").and_then(|v| v.as_str()) == Some("heartbeat") {
                            if let Ok(mut p) = peers.try_lock() {
    ...
    async fn handle_incoming(
        ...
        if first.payload.get("type").and_then(|v| v.as_str()) != Some("hello") {
